...
**ish** is a general-purpose programming language where every feature sits on a **streamlined ↔ encumbered** continuum. Start fast and loose like a dynamic language. Add constraints incrementally until your code is as strict as Rust. Choose per-feature, per-module, or per-project.

## Why ish?

Most languages pick one point on the static↔dynamic spectrum. ish lets you choose — and change your mind — per feature:

- **Streamlined**: Interpreted, garbage-collected, structurally typed. Feels like Python or Lua.
- **Encumbered**: Compiled, memory-managed, nominally typed. Feels like Rust or TypeScript at its strictest.
- **Mixed**: Different parts of the same codebase at different points on the continuum.

Encumbering code doesn't make it faster — it makes hidden concerns visible, so you can address them. See [docs/comparison.md](docs/comparison.md) for how ish relates to other languages.
...
| Section | Description |
|---------|-------------|
| [Language Specification](docs/spec/INDEX.md) | Normative rules: types, modules, agreement, execution |
...
|---------|---------|---------|
| Encumbrance | Per-feature constraints on a streamlined↔encumbered axis | [docs/spec/agreement.md](docs/spec/agreement.md) |
| Agreement | Protocol for how code negotiates constraints across boundaries | [docs/spec/agreement.md](docs/spec/agreement.md) |
| Execution configs | Dev/test/prod profiles that enforce encumbrance levels | [docs/spec/execution.md](docs/spec/execution.md) |

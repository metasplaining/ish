...
fn register_io(env: &Environment) {
    ...
    env.define(
        ...
        new_builtin("read_file", |args| {
            if args.len() != 1 {
                return Err(RuntimeError::new("read_file expects 1 argument"));
            }
            if let Value::String(path) = &args[0] {
                match std::fs::read_to_string(path.as_ref()) {
                    Ok(content) => Ok(Value::String(Rc::new(content))),
                    Err(e) => Err(RuntimeError::new(format!("read_file error: {}", e))),
                }
            } else {
                Err(RuntimeError::new("read_file expects a string path"))
            }
        }),
    );
    ...
    env.define(
        ...
        new_builtin("write_file", |args| {
            if args.len() != 2 {
                return Err(RuntimeError::new("write_file expects 2 arguments"));
            }
            if let (Value::String(path), Value::String(content)) = (&args[0], &args[1]) {
                match std::fs::write(path.as_ref(), content.as_ref()) {
                    Ok(()) => Ok(Value::Null),
                    Err(e) => Err(RuntimeError::new(format!("write_file error: {}", e))),
                }
            } else {
                Err(RuntimeError::new(
                    "write_file expects (string path, string content)",
                ))
            }
        }),
    );
...
fn register_strings(env: &Environment) {
    env.define(
        ...
        new_builtin("str_concat", |args| {
            if args.len() != 2 {
                return Err(RuntimeError::new("str_concat expects 2 arguments"));
            }
    ...
    env.define(
        ...
        new_builtin("str_length", |args| {
            if args.len() != 1 {
                return Err(RuntimeError::new("str_length expects 1 argument"));
            }
            if let Value::String(s) = &args[0] {
                Ok(Value::Int(s.len() as i64))
            } else {
                Err(RuntimeError::new("str_length expects a string"))
            }
        }),
    );
    ...
    env.define(
        ...
        new_builtin("str_slice", |args| {
            if args.len() != 3 {
                return Err(RuntimeError::new("str_slice expects 3 arguments"));
            }
                ...
                (&args[0], &args[1], &args[2])
            {
                ...
                Err(RuntimeError::new("str_slice expects (string, int, int)"))
            }
        }),
    );
    ...
    env.define(
        ...
        new_builtin("str_contains", |args| {
            if args.len() != 2 {
                return Err(RuntimeError::new("str_contains expects 2 arguments"));
            }
            if let (Value::String(s), Value::String(substr)) = (&args[0], &args[1]) {
                Ok(Value::Bool(s.contains(substr.as_ref())))
            } else {
                Err(RuntimeError::new("str_contains expects (string, string)"))
            }
        }),
    );
    ...
    env.define(
        ...
        new_builtin("str_starts_with", |args| {
            if args.len() != 2 {
                return Err(RuntimeError::new("str_starts_with expects 2 arguments"));
            }
            if let (Value::String(s), Value::String(prefix)) = (&args[0], &args[1]) {
                Ok(Value::Bool(s.starts_with(prefix.as_ref())))
            } else {
                Err(RuntimeError::new(
                    "str_starts_with expects (string, string)",
                ))
            }
        }),
    );
    ...
    env.define(
        ...
        new_builtin("str_replace", |args| {
            if args.len() != 3 {
                return Err(RuntimeError::new("str_replace expects 3 arguments"));
            }
    ...
    env.define(
        ...
        new_builtin("str_split", |args| {
            if args.len() != 2 {
                return Err(RuntimeError::new("str_split expects 2 arguments"));
            }
            if let (Value::String(s), Value::String(delim)) = (&args[0], &args[1]) {
                ...
                Err(RuntimeError::new("str_split expects (string, string)"))
            }
        }),
    );
    ...
    env.define(
        ...
        new_builtin("str_to_upper", |args| {
            if args.len() != 1 {
                return Err(RuntimeError::new("str_to_upper expects 1 argument"));
            }
            if let Value::String(s) = &args[0] {
                Ok(Value::String(Rc::new(s.to_uppercase())))
            } else {
                Err(RuntimeError::new("str_to_upper expects a string"))
            }
        }),
    );
    ...
    env.define(
        ...
        new_builtin("str_to_lower", |args| {
            if args.len() != 1 {
                return Err(RuntimeError::new("str_to_lower expects 1 argument"));
            }
            if let Value::String(s) = &args[0] {
                Ok(Value::String(Rc::new(s.to_lowercase())))
            } else {
                Err(RuntimeError::new("str_to_lower expects a string"))
            }
        }),
    );
    ...
    env.define(
        ...
        new_builtin("str_char_at", |args| {
            if args.len() != 2 {
                return Err(RuntimeError::new("str_char_at expects 2 arguments"));
            }
            if let (Value::String(s), Value::Int(i)) = (&args[0], &args[1]) {
                ...
                Err(RuntimeError::new("str_char_at expects (string, int)"))
            }
        }),
    );
    ...
    env.define(
        ...
        new_builtin("str_trim", |args| {
            if args.len() != 1 {
                return Err(RuntimeError::new("str_trim expects 1 argument"));
            }
            if let Value::String(s) = &args[0] {
                Ok(Value::String(Rc::new(s.trim().to_string())))
            } else {
                Err(RuntimeError::new("str_trim expects a string"))
            }
        }),
    );
}
...
fn register_lists(env: &Environment) {
    env.define(
        ...
        new_builtin("list_push", |args| {
            if args.len() != 2 {
                return Err(RuntimeError::new("list_push expects 2 arguments"));
            }
            if let Value::List(list_ref) = &args[0] {
                list_ref.borrow_mut().push(args[1].clone());
                Ok(Value::Null)
            } else {
                Err(RuntimeError::new("list_push expects a list as first argument"))
            }
        }),
    );
    ...
    env.define(
        ...
        new_builtin("list_pop", |args| {
            if args.len() != 1 {
                return Err(RuntimeError::new("list_pop expects 1 argument"));
            }
            if let Value::List(list_ref) = &args[0] {
                Ok(list_ref.borrow_mut().pop().unwrap_or(Value::Null))
            } else {
                Err(RuntimeError::new("list_pop expects a list"))
            }
        }),
    );
    ...
    env.define(
        ...
        new_builtin("list_length", |args| {
            if args.len() != 1 {
                return Err(RuntimeError::new("list_length expects 1 argument"));
            }
            if let Value::List(list_ref) = &args[0] {
                Ok(Value::Int(list_ref.borrow().len() as i64))
            } else {
                Err(RuntimeError::new("list_length expects a list"))
            }
        }),
    );
    ...
    env.define(
        ...
        new_builtin("list_get", |args| {
            if args.len() != 2 {
                return Err(RuntimeError::new("list_get expects 2 arguments"));
            }
            if let (Value::List(list_ref), Value::Int(i)) = (&args[0], &args[1]) {
                ...
                Err(RuntimeError::new("list_get expects (list, int)"))
            }
        }),
    );
    ...
    env.define(
        ...
        new_builtin("list_set", |args| {
            if args.len() != 3 {
                return Err(RuntimeError::new("list_set expects 3 arguments"));
            }
            if let (Value::List(list_ref), Value::Int(i)) = (&args[0], &args[1]) {
                ...
                Err(RuntimeError::new("list_set expects (list, int, value)"))
            }
        }),
    );
    ...
    env.define(
        ...
        new_builtin("list_slice", |args| {
            if args.len() != 3 {
                return Err(RuntimeError::new("list_slice expects 3 arguments"));
            }
                ...
                (&args[0], &args[1], &args[2])
            {
                ...
                Err(RuntimeError::new("list_slice expects (list, int, int)"))
            }
        }),
    );
    ...
    env.define(
        ...
        new_builtin("list_join", |args| {
            if args.len() != 2 {
                return Err(RuntimeError::new("list_join expects 2 arguments"));
            }
            if let (Value::List(list_ref), Value::String(sep)) = (&args[0], &args[1]) {
                ...
                Err(RuntimeError::new("list_join expects (list, string)"))
            }
        }),
    );
}
...
fn register_objects(env: &Environment) {
    env.define(
        ...
        new_builtin("obj_get", |args| {
            if args.len() != 2 {
                return Err(RuntimeError::new("obj_get expects 2 arguments"));
            }
            if let (Value::Object(obj_ref), Value::String(key)) = (&args[0], &args[1]) {
                ...
                Err(RuntimeError::new("obj_get expects (object, string)"))
            }
        }),
    );
    ...
    env.define(
        ...
        new_builtin("obj_set", |args| {
            if args.len() != 3 {
                return Err(RuntimeError::new("obj_set expects 3 arguments"));
            }
            if let (Value::Object(obj_ref), Value::String(key)) = (&args[0], &args[1]) {
                obj_ref
                    .borrow_mut()
                ...
                Err(RuntimeError::new("obj_set expects (object, string, value)"))
            }
        }),
    );
    ...
    env.define(
        ...
        new_builtin("obj_has", |args| {
            if args.len() != 2 {
                return Err(RuntimeError::new("obj_has expects 2 arguments"));
            }
            if let (Value::Object(obj_ref), Value::String(key)) = (&args[0], &args[1]) {
                Ok(Value::Bool(obj_ref.borrow().contains_key(key.as_ref())))
            } else {
                Err(RuntimeError::new("obj_has expects (object, string)"))
            }
        }),
    );
    ...
    env.define(
        ...
        new_builtin("obj_keys", |args| {
            if args.len() != 1 {
                return Err(RuntimeError::new("obj_keys expects 1 argument"));
            }
            if let Value::Object(obj_ref) = &args[0] {
                ...
                Err(RuntimeError::new("obj_keys expects an object"))
            }
        }),
    );
    ...
    env.define(
        ...
        new_builtin("obj_values", |args| {
            if args.len() != 1 {
                return Err(RuntimeError::new("obj_values expects 1 argument"));
            }
            if let Value::Object(obj_ref) = &args[0] {
                let values: Vec<Value> = obj_ref.borrow().values().cloned().collect();
                Ok(new_list(values))
            } else {
                Err(RuntimeError::new("obj_values expects an object"))
            }
        }),
    );
    ...
    env.define(
        ...
        new_builtin("obj_remove", |args| {
            if args.len() != 2 {
                return Err(RuntimeError::new("obj_remove expects 2 arguments"));
            }
            if let (Value::Object(obj_ref), Value::String(key)) = (&args[0], &args[1]) {
                let removed = obj_ref
                    .borrow_mut()
                ...
                Err(RuntimeError::new("obj_remove expects (object, string)"))
            }
        }),
    );
}
...
fn register_types(env: &Environment) {
    env.define(
        ...
        new_builtin("type_of", |args| {
            if args.len() != 1 {
                return Err(RuntimeError::new("type_of expects 1 argument"));
            }
    ...
    env.define(
        ...
        new_builtin("is_type", |args| {
            if args.len() != 2 {
                return Err(RuntimeError::new("is_type expects 2 arguments"));
            }
            if let Value::String(type_name) = &args[1] {
                Ok(Value::Bool(args[0].type_name() == type_name.as_ref()))
            } else {
                Err(RuntimeError::new("is_type expects (value, string)"))
            }
        }),
    );
}
...
fn register_conversion(env: &Environment) {
    env.define(
        ...
        new_builtin("to_string", |args| {
            if args.len() != 1 {
                return Err(RuntimeError::new("to_string expects 1 argument"));
            }
    ...
    env.define(
        ...
        new_builtin("to_int", |args| {
            if args.len() != 1 {
                return Err(RuntimeError::new("to_int expects 1 argument"));
            }
            match &args[0] {
    ...
    env.define(
        ...
        new_builtin("to_float", |args| {
            if args.len() != 1 {
                return Err(RuntimeError::new("to_float expects 1 argument"));
            }
            match &args[0] {

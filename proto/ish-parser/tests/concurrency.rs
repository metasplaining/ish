...
fn parse_await_expression() {
    ...
    match &program.statements[0] {
        Statement::ExpressionStmt(Expression::Await { callee, args }) => {
...
// ── Grammar restriction: await/spawn require call expression (TODO 49) ──
...
#[test]
fn parse_await_non_call_produces_incomplete() {
    let program = ish_parser::parse("await 42").unwrap();
    match &program.statements[0] {
        Statement::Incomplete { kind } => {
            assert_eq!(*kind, IncompleteKind::AwaitNonCall);
        }
        Statement::ExpressionStmt(Expression::Incomplete { kind }) => {
            assert_eq!(*kind, IncompleteKind::AwaitNonCall);
        }
        other => panic!("expected Incomplete(AwaitNonCall), got {:?}", other),
    }
}

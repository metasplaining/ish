...
output=$(run_ish 'fn work() { return 42 }; let f = spawn work(); println(f == f)')
assert_output "future reflexive equality" "true" "$output"

# --- Two independent futures are not equal ---
output=$(run_ish 'fn work() { return 42 }; let f1 = spawn work(); let f2 = spawn work(); println(f1 == f2)')
assert_output "independent futures not equal" "false" "$output"

# --- Cloned future reference is equal ---
output=$(run_ish 'fn work() { return 42 }; let f = spawn work(); let g = f; println(f == g)')

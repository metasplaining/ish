...
output=$(run_ish 'fn compute() { return 2 + 3 }; println(await compute())')
...
output=$(run_ish 'fn id(x: int) { return x }; let results = {a: 0, b: 0, c: 0}; results.a = await id(1); results.b = await id(2); results.c = await id(3); println(results.a + results.b + results.c)')

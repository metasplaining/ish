...
| **Compiled function** | An `IshFunction` whose shim was provided directly (not created by the VM from an interpreted body). Builtins and future stdlib functions are compiled functions. See [docs/architecture/runtime.md](docs/architecture/runtime.md). |
| **Core crate** | The `ish-core` crate: shared types used by both `ish-ast` and `ish-runtime`, primarily `TypeAnnotation`. See [docs/architecture/overview.md](docs/architecture/overview.md). |
...
| **Error code** | A type-safe enum (`ErrorCode`) in `ish-runtime` with variants E001–E013, identifying the category of a `RuntimeError`. See [docs/spec/errors.md](docs/spec/errors.md). |
...
| **Function implementation** | All functions have a shim. The `FunctionImplementation` enum has been eliminated. When the VM declares an interpreted function, it wraps the body in a shim closure that captures the environment, body statement, and VM reference. See [docs/architecture/vm.md](docs/architecture/vm.md). |
...
| **Runtime crate** | The `ish-runtime` crate: contains `Value`, `Shim`, `RuntimeError`, `ErrorCode`, `IshFunction`, and related types. All compiled ish packages depend on this crate. See [docs/architecture/runtime.md](docs/architecture/runtime.md). |
| **Shim function** | A synchronous Rust closure (`Fn(&[Value]) -> Result<Value, RuntimeError>`) that receives already-validated arguments, performs the function's work (or spawns it), and returns a `Value` (which may be `Value::Future` for yielding/parallel functions). All functions — builtins and interpreted — are implemented as shims. Also the entry point in a dynamically linked package that marshals calls between the ish runtime and compiled library code. See [docs/architecture/runtime.md](docs/architecture/runtime.md). |

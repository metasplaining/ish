...
- The unyielding execution path (`exec_statement_unyielding`, `eval_expression_unyielding`)
  ...
  is valid in an unyielding execution context (Decision 6).
...
- Replace "fn with spawn classified yielding" with "fn with spawn classified unyielding"
  (Decision 2 pending — default is replace):
  ```bash
...
The grammar is expanded so that `await` accepts any expression (Decision 3). At runtime, if
the expression does not evaluate to a `Value::Future`, ish throws E014 (Decision 4). The
`Await` AST node is restructured from `{callee, args}` to `{expr}` (Decision 5).
...
2. The test "`@[unyielding]` fn body containing spawn → analyzer error at declaration" is
   semantically wrong. Unyielding functions are allowed to call `spawn` (Decision 6).

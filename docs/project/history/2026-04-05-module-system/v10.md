...
*Derived from [module-system.md](module-system.md) v3 on 2026-04-05. Revised on 2026-04-05 (×6).*
...
Add `Statement::Bootstrap` evaluation:

1. Check that the caller file is not under any `project.json` in its hierarchy (using `module_loader::find_project_root`). If it is, return an error.
2. Parse the bootstrap config (path/URL/inline JSON).
3. Apply the config as the file's effective project context.
4. URL-based sources are fetched via `ISH_PROXY` (environment variable).

--> We don't actually do anything with the contents of the bootstrap file, yet.  `ISH_PROXY` is not yet fully defined.  Defer steps 2, 3, 4.

**Interface file consistency checking**

When loading a module file (during `Statement::Use` processing), check for a sibling `.ishi` file at the same path with `.ishi` extension instead of `.ish`. If one is found:
...
3. Emit granular errors:
   - `interface/symbol-not-in-implementation` — symbol in `.ishi` not found in `.ish`
   - `interface/symbol-not-in-interface` — `pub` symbol in `.ish` not declared in `.ishi`
   - `interface/symbol-mismatch` — symbol present in both but with different signatures

--> interpreter.rs is already to big.  Most or all of the additional behavior specified for it needs to go in new modules.
--> Now that forward declarations are supported, the code analyzer needs to be updated to support them.  Functions that call into other functions being declared at the same time need to be set to yielding if the function that they are calling is yielding.  If there is a cycle, and any of the functions is yielding then they all become yielding.  If none of them have been determined to be yielding on other criteria, then they all become unyielding.

**`lib.rs`**

Export the new `module_loader` and `access_control` modules.
...
--> Defer changes to ish-codegen for now.
...
| Code | Domain | Description | Production site |
|------|--------|-------------|----------------|
| `module/not-found` | module | `use` path has no matching `.ish` file | `module_loader::resolve_module_path` |
| `module/cycle` | module | Circular `use` dependency detected | `interpreter.rs` — Use evaluation |
| `module/script-not-importable` | module | File imported via `use` contains top-level commands | `interpreter.rs` — Use evaluation |
| `module/path-conflict` | module | Both `foo.ish` and `foo/index.ish` exist | `module_loader::resolve_module_path` |
| `module/declare-block-command` | module | `declare { }` block contains a non-declaration statement | `interpreter.rs` — DeclareBlock evaluation |
| `module/bootstrap-in-project` | module | `bootstrap` used inside a project hierarchy | `interpreter.rs` — Bootstrap evaluation |
| `interface/symbol-not-in-implementation` | interface | `.ishi` declares a symbol absent from the `.ish` file | `interpreter.rs` — interface check |
| `interface/symbol-not-in-interface` | interface | `.ish` has a `pub` symbol not declared in `.ishi` | `interpreter.rs` — interface check |
| `interface/symbol-mismatch` | interface | Symbol present in both `.ishi` and `.ish` with mismatched signatures | `interpreter.rs` — interface check |

--> Designate actual error codes for the new errors.  Update all of the places where error codes are defined.
...
### `CLAUDE.md` task table

Add a row to the task playbooks table:

| Working on modules | [docs/spec/modules.md](docs/spec/modules.md) |

--> CLAUDE.md and copilot instructions are symlinks to AGENTS.md.  Never update them directly.  Add a note about this to AGENTS.md.  Add this information to AGENTS.md instead.
...
- Where to look when adding a new module-system feature (module_loader.rs, access_control.rs, interpreter.rs Use handling)
- Acceptance test location for module tests (new test file in `proto/ish-tests/`)
...
**Decision:** Split into Proposal A-1 (Language Representation: `ish-core`, `ish-ast`, `ish-parser`, grammar, GLOSSARY, `docs/spec/syntax.md`, `docs/architecture/ast.md`) and Proposal A-2 (Execution and Tooling: `ish-vm`, `ish-codegen`, `ish-shell`, error catalog, user guide, acceptance tests, architecture docs)?

The natural boundary is between representation (parser and AST changes that other crates depend on) and execution (everything that operates on the AST). A-1 has no runtime dependencies; A-2 depends on A-1. Implementing A-1 first produces a complete, compilable parser and AST with correct node types that the VM implementation can then target.

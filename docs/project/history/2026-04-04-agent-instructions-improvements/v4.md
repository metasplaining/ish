---
title: "Agent Infrastructure — v4 (pre-revision snapshot)"
category: history
audience: [ai-dev]
status: snapshot
...
# Agent Infrastructure — v4 (pre-revision snapshot)

*Snapshot of `docs/project/proposals/agent-infrastructure.md` before revision on 2026-04-04.*
*Revised because: agent-pipeline-skills proposal has been implemented.*
...
2. Copy all skill files from `.claude/skills/` to `.agents/skills/` (preserving the per-skill subdirectory structure).
...
5. Verify that `/revise`, `/propose`, `/plan-implementation`, `/implement`, `/accept`, `/audit` all resolve correctly from both vendor contexts.
...
The new AGENTS.md combines all content from the current AGENTS.md and CLAUDE.md, adds the "Never Touch" section, expands the tech stack section, adds the no-backward-compatibility rule, and references the full set of skills from agent-pipeline-skills.md. Target: ~400–500 lines.
...
- **New skills from Proposal B**: AGENTS.md task playbooks must reference Bug Fix, Troubleshoot, and Verify. Implement Proposal B first so the skill paths are known.
...
**9. Task Playbooks** (update with new skills)
...
### Proposed Implementation (AGENTS.md write step)
...
2. Read docs/INDEX.md, the skills directory listing, and proto/Cargo.toml (crate list).
...
- `.agents/skills/` (new) — canonical skill location with all existing skills

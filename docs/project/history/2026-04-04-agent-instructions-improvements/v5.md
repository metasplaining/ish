---
title: "Agent Infrastructure — v5 (pre-revision snapshot)"
category: history
audience: [ai-dev]
status: snapshot
---

> Pre-revision snapshot of `docs/project/proposals/agent-infrastructure.md` taken 2026-04-05 before incorporating inline decisions on open questions in the AGENTS.md Maintenance Skill feature.

<!-- content below is verbatim from the proposal at time of snapshot -->
...
- **All nine skills now known**: The three new skills (Bug Fix, Troubleshoot, Verify) are at `.claude/skills/bug-fix/`, `.claude/skills/troubleshoot/`, `.claude/skills/verify/` prior to migration. After migration, all nine skills are at `.agents/skills/<name>/`. Write AGENTS.md with the post-migration paths.
...
**9. Task Playbooks** (full nine-skill set plus update-agents)
...
**Open Question:** Step 2 reads `docs/INDEX.md` — what should the skill do if this file is missing: skip it silently, report the gap to the human, or abort?
--> Report the gap to a human and abort.

**Open Question:** What acceptance test verifies that `/update-agents` correctly detects a skill in `.agents/skills/` that is absent from the Task Playbooks table?
--> Omit an acceptance test of this requirement.

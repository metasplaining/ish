...
depends-on: [docs/spec/assurance-ledger.md, docs/spec/polymorphism.md]
---
...
The ish language processor chooses the highest-performing model for which all constraints are met. For example, interpreted ish always uses garbage collection. The stack is used when a variable's size is known at build time and the variable only exists for the lifetime of a single function call. The active standard can require a specific model and produce a discrepancy if the constraints for that model are not met.
...
- [ ] **Ownership and borrowing rules.** Are there borrowing / lifetime annotations? Is there a borrow checker in high-assurance mode? Rules for passing owned values to functions?
